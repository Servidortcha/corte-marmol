...
function dibujoRefSize() {
  const ref = Math.max(100, parseFloat(document.getElementById("dibujoRef").value) || 3000);
  const wrap = document.querySelector(".dibujo-wrap");
  const rect = wrap ? wrap.getBoundingClientRect() : { width: 800 };
  const aspect = rect.height > 0 ? rect.width / rect.height : 1.6;
  return { ref, height: Math.round(ref / aspect) };
}

function dibujoRender(prevX, prevY) {
  const svg = document.getElementById("dibujoCanvas");
  const { ref, height } = dibujoRefSize();
  const ns = "http://www.w3.org/2000/svg";
  svg.setAttribute("viewBox", `0 0 ${ref} ${height}`);
  ...
  bg.setAttribute("width", ref); bg.setAttribute("height", height);
  bg.setAttribute("fill", "#f6f7f8"); bg.setAttribute("stroke", "#c8cdd3");
  svg.appendChild(bg);

  const pts = dibujoPuntos;
  if (pts.length >= 2) {
    const draw = [...pts];
    if (pts.length >= 3 && prevX == null) draw.push(pts[0]);
    const path = document.createElementNS(ns, "path");
    path.setAttribute("d", ringPath(draw));
    path.setAttribute("fill", "#7aa3d6"); path.setAttribute("fill-opacity", 0.25);
    path.setAttribute("stroke", "#2c5aa0"); path.setAttribute("stroke-width", 3);
    svg.appendChild(path);
  }
  if (prevX != null && pts.length >= 1) {
    const line = document.createElementNS(ns, "line");
    line.setAttribute("x1", pts[pts.length - 1][0]); line.setAttribute("y1", pts[pts.length - 1][1]);
    line.setAttribute("x2", prevX); line.setAttribute("y2", prevY);
    line.setAttribute("stroke", "#2c5aa0"); line.setAttribute("stroke-dasharray", "6 5");
    svg.appendChild(line);
  }
  pts.forEach(([x, y], i) => {
    const c = document.createElementNS(ns, "circle");
    c.setAttribute("cx", x); c.setAttribute("cy", y); c.setAttribute("r", 6);
    c.setAttribute("fill", "#fff"); c.setAttribute("stroke", "#2c5aa0");
    c.setAttribute("stroke-width", 3);
    svg.appendChild(c);
    const t = document.createElementNS(ns, "text");
    t.setAttribute("x", x + 8); t.setAttribute("y", y - 8);
...
function dibujoPtsRender() {
  const container = document.getElementById("dibujoPts");
  const { ref, height } = dibujoRefSize();
  container.innerHTML = "<div class='dibujo-labels'><span>#</span><span>X (mm)</span><span>Y (mm)</span><span></span></div>";
  if (dibujoPuntos.length === 0) {
    document.getElementById("dibujoMedidas").textContent = "0 × 0 mm";
    return;
  }
  dibujoPuntos.forEach((pt, i) => {
    const row = document.createElement("div");
    row.className = "dibujo-row";
    row.innerHTML =
      `<span>${i + 1}</span>` +
      `<input type="number" class="ptx" value="${pt[0]}">` +
      `<input type="number" class="pty" value="${pt[1]}">` +
      `<button class="del">&times;</button>`;
    row.querySelector(".ptx").addEventListener("change", (e) => {
      dibujoPuntos[i][0] = parseFloat(e.target.value) || 0;
      dibujoRender();
    });
    row.querySelector(".pty").addEventListener("change", (e) => {
      dibujoPuntos[i][1] = parseFloat(e.target.value) || 0;
      dibujoRender();
    });
    row.querySelector(".del").addEventListener("click", () => {
      dibujoPuntos.splice(i, 1);
      dibujoRender();
    });
    container.appendChild(row);
  });
  const xs = dibujoPuntos.map((p) => p[0]);
  const ys = dibujoPuntos.map((p) => p[1]);
  const w = Math.max(0, Math.max(...xs) - Math.min(...xs));
  const h = Math.max(0, Math.max(...ys) - Math.min(...ys));
  document.getElementById("dibujoMedidas").textContent = `${w} × ${h} mm`;
}

function dibujoInit() {
  const svg = document.getElementById("dibujoCanvas");
  const refSize = dibujoRefSize();
  svg.addEventListener("click", (e) => {
    const rect = svg.getBoundingClientRect();
    const { ref, height } = dibujoRefSize();
    const x = ((e.clientX - rect.left) / rect.width) * ref;
    const y = ((e.clientY - rect.top) / rect.height) * height;
    dibujoPuntos.push([Math.round(x), Math.round(y)]);
    if (dibujoPuntos.length > 40) dibujoPuntos = dibujoPuntos.slice(-40);
    dibujoRender();
  });
  ...
  svg.addEventListener("mouseleave", () => dibujoRender());
  document.getElementById("dibujoRef").addEventListener("change", () => dibujoRender());
  document.getElementById("dibujoUndo").addEventListener("click", () => {
    dibujoPuntos.pop();
    dibujoRender();
  });
  document.getElementById("dibujoClear").addEventListener("click", () => {
    dibujoPuntos = [];
    dibujoRender();
  });
  document.getElementById("dibujoUsar").addEventListener("click", async () => {
    const error = document.getElementById("dibujoError");
    error.hidden = true;
    if (dibujoPuntos.length < 3) {
      error.hidden = false;
      error.textContent = "Marcá al menos 3 puntos en el lienzo.";
      return;
    }
    const pts = dibujoPuntos.map((p) => [p[0], p[1]]);
    dibujoCount += 1;
    try {
      const slab = await planchaFromPiece(`Retazo dibujado ${dibujoCount}`, pts, []);
      ...
      error.hidden = false;
      error.textContent = "No se pudo agregar la plancha: " + err.message;
    }
  });
  dibujoRender();
}

function renumberPriorities() {  ["#pieceRows", "#slabRows"].forEach((selector) => {
